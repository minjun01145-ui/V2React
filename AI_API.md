...
브라우저의 `ai-tutor-engine`은 전송 계약, 응답 검증, 공통 채점 결과만 소유합니다. 구체 게임의 세트 변환과 UI는 `games/ai-tutor`에 있습니다. 서버의 `ai-tutor` 계층은 현재 라운드·참가자·세트·문항을 직접 검증하고, 학생에게서 system prompt나 기준 답안을 받지 않습니다.
