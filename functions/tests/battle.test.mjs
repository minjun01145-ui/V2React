...
import { parseBattleGameConfig, resolveBattleQuestionSide } from "../lib/battle/config.js";
...
assert.deepEqual(parseBattleGameConfig({}), { direction: "free", answerDurationMs: 20_000 });
...
assert.deepEqual(parseBattleGameConfig({
  "battle-direction": "bad",
  "battle-answer-seconds": "99",
}), { direction: "free", answerDurationMs: 20_000 });
assert.equal(resolveBattleQuestionSide("translation-only", "source"), "meaning");
assert.equal(resolveBattleQuestionSide("translation-only", "meaning"), "meaning");
assert.equal(resolveBattleQuestionSide("composition-only", "source"), "source");
assert.equal(resolveBattleQuestionSide("composition-only", "meaning"), "source");
assert.equal(resolveBattleQuestionSide("free", "meaning"), "meaning");
...
const choosingMatch = {
  memberIds: ["a", "b"], memberProfiles: [], hearts: { a: 2, b: 2 }, usedItemIds: [], itemCount: 10,
  questionNumber: 0, attackerIndex: 0, defenderIndex: 1, generation: 1, phase: "choosing", deadlineAtMs: 1,
  selectedItemId: null, selectedSide: null, prompt: null, expectedAnswer: null, gradingSubmissionId: null,
  eventRevision: 0, rewardItemId: null, inkBlockedUntilAtMs: { a: 0, b: 0 },
};
