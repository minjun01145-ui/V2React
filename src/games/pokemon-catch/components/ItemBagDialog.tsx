...
import { captureChancePercent } from "../captureRules.ts";
...
interface Props {
  ...
  readonly ballChances: Readonly<Partial<Record<PokemonItemId, number>>>;
  ...
  return <div className={styles.modalBackdrop} role="dialog" aria-modal="true" aria-label="아이템 가방"><section className={styles.itemCard}>
    ...
    <div className={styles.itemGrid}>{POKEMON_ITEMS.map((item) => {
      ...
      return <div key={item.id} data-item={item.id}>
        ...
        <div><strong>{item.name}</strong><small>{item.kind === "ball" && ballChances[item.id] !== undefined ? `${item.description} · 현재 약 ${captureChancePercent(ballChances[item.id]!)}%` : item.description}</small></div><b>× {inventory[item.id]}</b>
