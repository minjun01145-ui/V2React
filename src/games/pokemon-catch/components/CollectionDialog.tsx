...
export function CollectionDialog({ captures, onClose }: Props) {
  return <div className={styles.modalBackdrop} role="dialog" aria-modal="true" aria-label="내 포획함"><section className={styles.collectionCard}>
    ...
    {captures.length === 0 ? <p className={styles.emptyCollection}>아직 잡은 포켓몬이 없어요.</p> : <div className={styles.collectionGrid}>{captures.map((pokemon) => <div key={pokemon.captureId}><PokemonSprite pokemon={pokemon} alt="" /><strong>{pokemon.name}</strong><small>No.{String(pokemon.speciesId).padStart(3, "0")}</small></div>)}</div>}
  </section></div>;
}
