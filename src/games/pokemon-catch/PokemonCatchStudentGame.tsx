...
export default function PokemonCatchStudentGame({ roomId, session, player }: StudentGameModuleProps) {
  const learningSet = usePokemonCatchSet(session);
  if (learningSet.loading) return <StatusPanel title="단어 세트 불러오는 중">야생 포켓몬과 문제를 준비하고 있습니다.</StatusPanel>;
  ...
  return <StudentPokemonCatch roomId={roomId} session={session} player={player} set={learningSet.set} />;
}
