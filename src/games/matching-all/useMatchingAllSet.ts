...
import { matchingAllDemoSet } from "./demoSet.ts";
...
export function useMatchingAllSet(session: ActiveGameSession): {
  ...
  return {
    set: (inlineSet as LearningSet | undefined) ?? remote.set ?? (setId ? null : matchingAllDemoSet),
