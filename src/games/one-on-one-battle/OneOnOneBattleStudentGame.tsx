...
export default function OneOnOneBattleStudentGame({
      ...
      if (eventKey.current !== nextEventKey) {
        ...
        if (state.eventType === "ink-used") {
          effects.play(createGameAnnouncement({
            ...
            metric: state.eventWasMine ? "상대 화면을 5초 가립니다" : "5초 동안 화면이 가려집니다",
            tone: "warning",
          }));
        }
  ...
  return <div className={styles.shell}>
    ...
    <section className={styles.itemBar} aria-label="배틀 아이템">
      <button
        type="button"
      ...
      <small>
        {canUseInk
          ? "지금 사용하면 상대 화면을 5초 가립니다."
          : inkCount > 0
            ? "상대가 문제를 고르거나 답하는 동안 사용할 수 있습니다."
            : "정답 보상에서 먹물을 획득할 수 있습니다."}
      </small>
    </section>

    {state.rewardAvailable && state.phase === "answering"
      ? <div className={styles.treasure}>🎁 이번 문제를 맞히면 먹물 아이템을 획득합니다!</div>
      : null}
