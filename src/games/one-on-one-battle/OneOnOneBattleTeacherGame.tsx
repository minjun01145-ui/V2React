...
export default function OneOnOneBattleTeacherGame({ roomId, session }: TeacherGameModuleProps) {
  ...
  return <div className={styles.teacher}><div className={styles.teacherHeader}><div><span>ONE ON ONE BATTLE</span><h2>1:1 배틀 현황</h2></div><TimedGameStatus session={session} compact /></div>
    {standings.loading ? <StatusPanel title="배틀을 편성하고 있습니다" tone="waiting">학생들을 익명으로 매칭하고 있습니다.</StatusPanel> : null}
    <div className={styles.standings}>{standings.value.map((student) => <Card className={styles.standing} key={student.id}><header><h3>{student.nickname}</h3><span className={styles.status}>{statusLabel[student.status]}</span></header><dl><div><dt>KILLS</dt><dd>{student.kills}</dd></div><div><dt>DEATHS</dt><dd>{student.deaths}</dd></div></dl></Card>)}</div>
  </div>;
}
