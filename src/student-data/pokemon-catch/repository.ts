...
function parseCapture(id: string, data: DocumentData): StoredCapturedPokemon | null {
  ...
  return {
    ...
    level: Math.max(1, count(data.level) || 1),
    types: Array.isArray(data.types) ? data.types.filter((type): type is string => typeof type === "string" && Boolean(type.trim())).slice(0, 2) : [],
    description: typeof data.description === "string" ? data.description : "",
    heightMeters: typeof data.heightMeters === "number" && Number.isFinite(data.heightMeters) && data.heightMeters > 0 ? data.heightMeters : null,
    weightKg: typeof data.weightKg === "number" && Number.isFinite(data.weightKg) && data.weightKg > 0 ? data.weightKg : null,
    spriteUrl,
...
export async function saveCapturedPokemon(accountId: string, capture: StoredCapturedPokemon): Promise<void> {
  await setDoc(doc(capturesCollection(accountId), capture.captureId), {
    ...
    heightMeters: capture.heightMeters,
