...
root.render(<StrictMode><AuthStatusPage title="수업을 준비하고 있어요" message="잠시만 기다려 주세요." /></StrictMode>);

void import("./StudentApp.tsx")
  .then(({ default: StudentApp }) => {
    root.render(<StrictMode><StudentApp /></StrictMode>);
  })
  .catch((error: unknown) => {
    const message = error instanceof Error ? error.message : "앱을 불러오지 못했습니다.";
    root.render(<StrictMode><AuthStatusPage title="사이트를 시작할 수 없어요" message="환경 설정을 확인한 뒤 다시 시도해 주세요." error={message} /></StrictMode>);
  });
