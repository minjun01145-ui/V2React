...
function firstText(...values: readonly unknown[]): string { for (const value of values) if (typeof value === "string" && value.trim()) return value.trim(); return ""; }
function extractChunks(item: Record<string, unknown>): string[] {
  const candidate = item.chunks ?? item.parts ?? item.segments ?? item.englishParts;
  if (Array.isArray(candidate)) return candidate.map((chunk) => isRecord(chunk) ? firstText(chunk.text, chunk.value, chunk.label) : String(chunk ?? "").trim()).filter(Boolean);
  const sentence = firstText(item.sourceText, item.chunkedEnglish, item.english, item.en, item.sentence);
  return sentence.includes("/") ? sentence.split("/").map((chunk) => chunk.trim()).filter(Boolean) : [];
}
