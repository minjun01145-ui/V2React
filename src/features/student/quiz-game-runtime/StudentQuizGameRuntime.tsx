...
export default function StudentQuizGameRuntime({ roomId, session, player, quizGame }: {
  ...
  return <section className={styles.runtime}>
    ...
    {quiz.phase === "submissions" ? <><StatusPanel title="답안 제출이 끝났어요">{round.source.kind === "free-response" ? "선생님이 제출된 답안을 확인하고 있습니다." : "친구들의 제출 상태를 확인하고 있습니다."}</StatusPanel>{round.source.kind === "custom" && presenter ? <Card><h2>정답 확인</h2><div className={styles.answers}>{round.source.items.map((item, index) => { const question = presenter(item, round.gameConfig); return <div key={item.id}><b>{index + 1}</b><span>{question.prompt}</span><strong>{question.answer}</strong></div>; })}</div></Card> : null}</> : null}
