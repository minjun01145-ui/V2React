...
export default function TeacherQuizGamePage({ roomId }: { readonly roomId: string }) {
  ...
  const selectEngine = (round: QuizGameRound, gameId: string): void => {
    ...
    updateRound(round.id, (current) => ({
      ...current,
      gameId: game.id,
      source: game.id === "free-response" ? { kind: "free-response", prompt: "" } : { kind: "stored-set", setId: candidates[0]?.id ?? null },
      gameConfig: Object.fromEntries(game.settings.map((setting) => [setting.key, setting.defaultValue])),
    }));
  };
        ...
        <div className={styles.rounds}>{rounds.map((round, index) => {
          ...
          return <section className={styles.round} key={round.id}>
            ...
            <div className={styles.fields}>
              ...
              {source.kind !== "free-response" ? <label>문제 공급<select value={round.source.kind} onChange={(event) => updateRound(round.id, (current) => event.target.value === "custom" ? { ...current, source: { kind: "custom", setType: (game.supportedSetTypes[0] === "reading-chunks" ? "reading-chunks" : "vocabulary"), items: [newCustomItem()] } } : { ...current, source: { kind: "stored-set", setId: candidates[0]?.id ?? null } })} disabled={Boolean(busy) || !game.supportsFiniteQuizQuestions}><option value="stored-set">학습 세트 반복</option>{game.supportsFiniteQuizQuestions ? <option value="custom">교사 직접 출제</option> : null}</select></label> : null}
            ...
            {source.kind === "free-response" ? <label className={styles.promptField}>학생에게 보여줄 질문<textarea rows={3} value={source.prompt} maxLength={1000} placeholder="예: 오늘 배운 내용에서 가장 인상 깊었던 점은 무엇인가요?" disabled={Boolean(busy)} onChange={(event) => updateRound(round.id, (current) => current.source.kind === "free-response" ? { ...current, source: { ...current.source, prompt: event.target.value } } : current)} /><small>정답은 입력하지 않습니다. 학생은 마감 전까지 답안을 수정할 수 있습니다.</small></label> : null}
