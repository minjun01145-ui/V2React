...
type SupportedGameId = "pokemon-catch" | "sentence-builder";
...
export default function TeacherRoomController({ roomId, embedded = false }: Props) {
  ...
  const actions = <>
    <Button disabled={working || loading || isPlaying || activePlayers.length === 0 || invalidPokemonSet} onClick={() => void run((id) => startSession(id, { gameId, gameConfig: selectedSet ? { setId: selectedSet.id } : {} }))}>게임 시작</Button>
  ...
  if (!embedded) return <PageShell eyebrow="TEACHER" title="교사용 컨트롤" roomId={roomId} actions={actions}>{content}</PageShell>;
  return <section className={styles.embedded} aria-label="테스트 멀티플레이 제어">
    <header className={styles.embeddedHeader}><div><Eyebrow>REAL MULTIPLAYER</Eyebrow><h2>테스트 대기실 제어</h2></div><div className={styles.actions}>{actions}</div></header>
    {content}
  </section>;
}
