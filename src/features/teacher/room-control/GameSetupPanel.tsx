...
export function GameSetupPanel({ setup, disabled }: Props) {
  ...
  return <div className={styles.gameSetup}>
    ...
    <div className={styles.pickerControls}>
      ...
      {selectedGame.timing === "timed" && selectedGame.fixedTimedMode === null ? <label>게임 시간<select value={timedMode} onChange={(event) => { if (isTimedGameMode(event.target.value)) setup.selectTimedMode(event.target.value); }} disabled={disabled}>{TIMED_GAME_MODE_OPTIONS.map((option) => <option value={option.mode} key={option.mode}>{option.label} 모드</option>)}</select></label> : null}
      {selectedGame.timing === "timed" && selectedGame.fixedTimedMode !== null ? <label>게임 시간<select value={selectedGame.fixedTimedMode} disabled><option value={selectedGame.fixedTimedMode}>{TIMED_GAME_MODE_OPTIONS.find((option) => option.mode === selectedGame.fixedTimedMode)?.label ?? "고정"} 고정</option></select></label> : null}
    </div>
